...
const EN = {
  ...
  aacNote: 'HE-AAC and HE-AAC v2 need the patent-encumbered libfdk_aac encoder, which cannot legally be shipped in a browser app — AAC here is AAC-LC. FFmpeg\'s own AAC encoder also has a narrower usable bitrate range than the format allows. Values it cannot reach at all are not offered; at 44.1 kHz stereo the steps above 224 kbps are, but they arrive short — 320 kbps comes out nearer 250 on music. Every result shows the bitrate actually delivered next to the one you set.',
...
const DE = {
  ...
  aacNote: 'HE-AAC und HE-AAC v2 benötigen den patentbelasteten Encoder libfdk_aac, der in einer Browser-App rechtlich nicht mitgeliefert werden darf — AAC ist hier AAC-LC. Der AAC-Encoder von FFmpeg hat außerdem einen engeren nutzbaren Bitratenbereich als das Format selbst. Werte, die er gar nicht erreicht, werden nicht angeboten; bei 44,1 kHz Stereo sind die Stufen über 224 kbps wählbar, kommen aber zu kurz an — 320 kbps landen bei Musik eher bei 250. Jedes Ergebnis zeigt die tatsächlich gelieferte Bitrate neben der eingestellten.',
